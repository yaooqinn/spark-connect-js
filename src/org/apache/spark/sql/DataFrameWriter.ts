...
export class DataFrameWriter {
   ...
   */
  save(path: string | undefined = undefined): Promise<b.ExecutePlanResponse[]> {
   ...
   */
  insertInto(tableName: string): Promise<b.ExecutePlanResponse[]> {
   ...
   *
   */
  saveAsTable(tableName: string): Promise<b.ExecutePlanResponse[]> {
    const setTable = (write: c.WriteOperation) => {
      write.saveType = {
        case: "table",
        value: create(c.WriteOperation_SaveTableSchema, {
   ...
   *   "READ_UNCOMMITTED".
   */
  jdbc(url: string, table: string, connectionProperties: {[key: string]: string}): Promise<b.ExecutePlanResponse[]> {
   ...
   *
   */
  json(path: string): Promise<b.ExecutePlanResponse[]> {
   ...
   * @since 1.0.0
   */
  parquet(path: string): Promise<b.ExecutePlanResponse[]> {
   ...
   * Source Option</a> in the version you use.
   */
  orc(path: string): Promise<b.ExecutePlanResponse[]> {
   ...
   * Data Source Option</a> in the version you use.
   */
  text(path: string): Promise<b.ExecutePlanResponse[]> {
   ...
   * Data Source Option</a> in the version you use.
   */
  csv(path: string): Promise<b.ExecutePlanResponse[]> {
   ...
   * Data Source Option</a> in the version you use.
   */
  xml(path: string): Promise<b.ExecutePlanResponse[]> {
    return this.format("xml").save(path);
  }

  private executeWriteOperation(f: (op: c.WriteOperation) => void): Promise<b.ExecutePlanResponse[]> {
